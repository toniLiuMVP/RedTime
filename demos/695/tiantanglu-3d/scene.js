...
const settings = { sens: 1, vol: 0.8, chSize: 8, chGap: 4, chThick: 2, chColor: "#7dff8a", quality: 3, difficulty: 1 };   // 預設劇情(300HP):確保第一次玩的人撐到放下槍→一眼瞬間的情緒核心
...
let cleared = false;   // 破關旗標(走完天堂路→一眼瞬間):持久化。一旦破關,劇情(1)/新手(2)永久關閉,只留普通/困難/天堂路(task4)
try { cleared = localStorage.getItem("tiantanglu_cleared_v1") === "1"; } catch (e) { }
if (cleared && settings.difficulty < 3) { settings.difficulty = 3; saveSettings(); }   // 破關後若還停在劇情/新手,夾到普通
let refreshDiffSlider = null;   // 由 bindDifficulty 設定:破關後把難度滑桿下限拉到普通
function markCleared() { if (cleared) return; cleared = true; try { localStorage.setItem("tiantanglu_cleared_v1", "1"); } catch (e) { } if (settings.difficulty < 3) { settings.difficulty = 3; saveSettings(); } if (refreshDiffSlider) refreshDiffSlider(); }
...
const MUSIC_MEMORIAL = "../一眼瞬間.mp3";   // 破關紀念續播:走完天堂路後不靜默跳回,讓一眼瞬間主題循環陪玩家邊聽邊看故事
...
function overlayOpen() { for (const id of ["settings", "shop", "diary", "tutorial", "story", "clear", "reader-embed"]) { const e = document.getElementById(id); if (e && e.classList.contains("on")) return true; } return false; }
...
document.addEventListener("pointerlockchange", () => { if (enterEl) enterEl.classList.toggle("hide", document.pointerLockElement === canvas); if (document.pointerLockElement !== canvas) { mouseDown = false; ads = false; } else if (MODE === "hub" && !firstHubShown) { firstHubShown = true; pupilT = 2.0; showNarr(NARR.hubEnter, 4.2); maybeFirstTutorial(); } });   // #1 首次進軍營:夢框建立句 + 首玩自動彈教學一次
...
(function bindDifficulty() {   // 難度 5 段:存設定,下次進實戰生效(避免戰鬥中改 HP)
  ...
  if (!el) return;
  const lo = () => (cleared ? 3 : 1);   // 破關後劇情/新手永久關閉,滑桿下限抬到正常
  const upd = () => { if (val) val.textContent = (DNAMES[settings.difficulty] || "正常") + (cleared ? " (已破關)" : ""); };
  const sync = () => { el.min = lo(); if (settings.difficulty < lo()) settings.difficulty = lo(); el.value = settings.difficulty; upd(); };
  sync();
  el.addEventListener("input", () => { settings.difficulty = Math.max(lo(), Math.min(5, Math.round(parseFloat(el.value)) || 3)); saveSettings(); upd(); });
  refreshDiffSlider = sync;   // 破關當下即時把滑桿夾到正常起跳
})();
...
(function bindTutorial() {   // 新手教學:cold-open 後進場畫面的友善教學(stopPropagation 不觸發進場)
  ...
  if (close && ov) close.addEventListener("click", (e) => { e.stopPropagation(); ov.classList.remove("on"); try { localStorage.setItem("tiantanglu_seen_tut_v1", "1"); } catch (e2) { } });
})();
function maybeFirstTutorial() {   // 首玩自動彈一次新手教學(主流玩家不會自己點 📖 → 看不懂 G 砲擊/B 軍械庫/靶場才開打就棄玩);看過記旗標不再彈
  let seen = false; try { seen = localStorage.getItem("tiantanglu_seen_tut_v1") === "1"; } catch (e) { }
  if (seen) return; const ov = document.getElementById("tutorial"); if (!ov) return;
  ov.classList.add("on"); if (document.pointerLockElement === canvas && document.exitPointerLock) document.exitPointerLock();
}
...
function fireTankShell(v, fx, fz) {
  const tip = new THREE.Vector3(v.group.position.x + fx * 3.6, 1.7, v.group.position.z + fz * 3.6);
  shake = Math.max(shake, 0.85); sfxExplode(tip); emitFx(sparkPool, tip, 0.35, 0.35, 1.1, true);
  const vel = new THREE.Vector3(fx, 0.02, fz).normalize().multiplyScalar(120);   // 改走真實彈道(spawnShell+updateShells):高初速平直快速,GRAV 自然微落;不再瞬間 hitscan
  spawnShell(tip, vel);
  tracer(tip.x, tip.y, tip.z, tip.x + fx * 16, tip.y + 0.32, tip.z + fz * 16, 0xffd060, 0.9);   // 砲口曳光指出彈道方向
}
const shells = [];
...
function updateVehicle(dt) {
  ...
  if (v.type === "howitzer") {   // 站位榴彈砲:自由瞄準 + 砲管俯仰 + 彈道發射
    ...
    if (hintEl) { hintEl.style.opacity = "1"; hintEl.textContent = (isTouch ? "榴彈砲 · 鏡頭抬高增加射程 · 射擊鈕發射 · 按「使用」鈕離開" : "榴彈砲 · 看高一點增加射程 · 左鍵發射 · 按 E 離開") + vehResHint(v); }
    return;
  }
  let thr = 0, steer = 0;
  ...
  if (v.fireT > 0) v.fireT -= dt;
  if (v.type === "tank" && mouseDown && v.fireT <= 0) { if (!sim || v.shells > 0) { v.fireT = ((curDiff && curDiff.diffIndex) || 3) >= 4 ? 2.8 : 1.5; if (sim) v.shells--; fireTankShell(v, fx, fz); } else { v.fireT = 0.4; sfxDry(); } }   // 困難/天堂路:戰車主砲放慢到接近真實裝填節奏,易模式維持原爽度
  if (hintEl) { const fireVerb = isTouch ? "射擊鈕開炮" : "左鍵開炮"; hintEl.textContent = (v.type === "tank" ? "戰車 · " + fireVerb + " · " + actHint("下車") : (dry ? "悍馬車 · 油料耗盡 · " + actHint("下車") : "悍馬車 · 撞擊殲敵 · " + actHint("下車"))) + vehResHint(v); }
}
...
function enemyShoot(g, dist) {
  ...
  if (hit) { const dm = ew.dmg; hurtPlayer((dm[0] + Math.random() * (dm[1] - dm[0])) * (dIdx >= 4 ? 1.2 : 1)); }   // 困難/天堂路:命中更痛
}
...
function renderStory(sec) {
  ...
  if (storyEl) storyEl.querySelectorAll(".story-tabs button").forEach((b) => b.classList.toggle("on", b.getAttribute("data-sec") === sec));
...
function openClear() {
  ...
  playMusic(MUSIC_MEMORIAL);   // 音樂續播(循環),不靜默跳回開頭
...
if (clearEl) {
  clearEl.querySelectorAll(".clear-actions button").forEach((b) => b.addEventListener("click", () => {
    ...
    if (act === "before") openStory("before");
    else if (act === "after") openStory("after");
    else if (act === "replay") { if (clearDiffEl) { clearDiffEl.hidden = false; if (clearDiffEl.scrollIntoView) clearDiffEl.scrollIntoView({ behavior: "smooth", block: "nearest" }); } }
    else if (act === "hub") { closeClear(); stopMusic(); }   // 回軍營:收紀念樂,回到 hub 自由走(難度已鎖在普通以上)
  }));
  clearEl.querySelectorAll(".clear-diff button").forEach((b) => b.addEventListener("click", () => {
    ...
    closeClear(); enterSim();   // 以選定難度(普通/困難/天堂路)重新踏上天堂路
  }));
}
...
function goalWave() { const d = (curDiff && curDiff.diffIndex) || 1; return d >= 4 ? 10 : d >= 3 ? 8 : 5; }   // 撐過這波=撐過了,夢讓他停下。toni 選按難度:劇情/新手 5、普通 8、困難/天堂路 10(越硬派越累才放得下)
...
let gazeMusic = null, gazeRAF = 0, gazeT0 = 0, gazeFigure = null, heartTimer = 0, gazeMusEnd = 14;
...
function startGaze() {
  ...
  gazeT0 = performance.now() + HOLD;   // 時間基準前移到「浮現起點」:留白期間 t 為負(心跳走 t<2.6 的慢沉拍),HOLD 結束 t 過 0 → 光形從 0 浮現;所有計時(心跳/reveal/字卡/收場)共用同一基準
  startHeartbeat();
  ...
  const beginReveal = () => {
    ...
    // 光形相:她浮現,「妳一直都在」(連接句 #6)
    setTimeout(() => { if (MODE === "gaze" && lineEl) lineEl.classList.add("show"); }, 4000);
    setTimeout(() => { if (MODE === "gaze" && lineEl) lineEl.classList.remove("show"); }, 8400);   // 淡掉,讓光形隨之淡入純黑
    setTimeout(() => stopHeartbeat(), 10200);   // 光形相結束→心跳收,文字相只留一眼瞬間樂
    ...
    const endAt = ct + 7000;   // 末句「每當我閉上眼睛」留白 7s 才收(餘韻拉長,讓空畫面被承受)
    gazeMusEnd = endAt / 1000;
    setTimeout(() => endGaze(), endAt);
    gazeLoop(ctx);   // gazeT0 已在開頭設好(= 浮現起點),beginReveal 此刻起跑 t≈0
  };
  if (HOLD > 0) setTimeout(beginReveal, HOLD); else beginReveal();
}
function gazeLoop(ctx) {
  ...
  const push = 1 + clarity * 0.06 + reveal * 0.02;   // ≤8% clarity-linked 緩推:她浮現完整那刻鏡頭微微逼近(被釘在原地那種看見),幅度小不致暈
  ...
  if (gazeMusic) { const env = Math.min(1, Math.max(0, (t - 1) / 2)) * Math.min(1, Math.max(0, (gazeMusEnd - t) / 3)); gazeMusic.volume = env * (settings.vol != null ? settings.vol : 0.8) * 0.45; }   // 一眼瞬間樂貫穿光形相+文字相:2s 淡入,末 3s 淡出,中間穩(不再隨光形淡掉)
}
function endGaze() {
  ...
  fadeBeat(0);
  // 純黑留白 ~4.5s(無 HUD/無樂,只剩漸消的心跳)→ 睜眼回 hub:「我醒了嗎,還是只是第一層夢醒了」(拉長讓末句「閉上眼睛」對應一段真的閉著的黑)
  setTimeout(() => {
    ...
    setTimeout(() => {
      ...
      updateWaveHUD();
      openClear();   // task4:走完天堂路不靜默跳回開頭→破關紀念(音樂續播、戰前/戰後故事、再次挑戰選難度)
    }, 1450);
  }, 4500);
}
...
function updateFP(dt) {
  ...
  if (hintEl) { if (MODE === "sim" && awaitDisarm && disarmT <= 0) { if (disarmReady) { hintEl.style.opacity = "1"; hintEl.classList.add("pulse"); hintEl.textContent = isTouch ? "夠了 · 輕觸「放下槍」" : "夠了 · 按 H 放下槍"; } else { hintEl.style.opacity = "0"; hintEl.classList.remove("pulse"); } } else { hintEl.classList.remove("pulse"); if (MODE === "sim" && inBreak && !shopPause && !(shopEl && shopEl.classList.contains("on"))) { hintEl.style.opacity = "1"; hintEl.textContent = wave >= 1 ? (isTouch ? "前一波已通關 · 按「軍械庫」鈕購買 · 倒數後自動開打" : "前一波已通關 · 按 B 開軍備購買 · 倒數後自動開打") : (isTouch ? "準備 · 按「軍械庫」鈕購買裝備" : "準備 · 按 B 開軍備購買"); } else if (MODE === "hub" && isActive()) { hintEl.style.opacity = "1"; hintEl.textContent = nearVehicle() ? actHint("上車駕駛") : nearCow() ? actHint("替連長顧那頭牛", " · ") : nearJacket() ? actHint("看那件反穿的外套", " · ") : nearDiary() ? actHint("翻開大兵日記 · 看戰前的故事", " · ") : nearRangeEntry() ? actHint("進入「實戰模擬練習」") : (isTouch ? "搖桿走動軍營 · 按「軍械庫」鈕購買裝備 · 走到靶場(右前方)按「使用」鈕開始實戰" : "自由走動軍營 · 按 B 開軍械庫購買裝備 · 走到靶場(右前方)按 E 開始實戰模擬"); } else hintEl.style.opacity = "0"; } }
  ...
  for (let i = 0; i < beacons.length; i++) { const b = beacons[i]; b.visible = MODE === "hub"; if (b.visible) { b.material.opacity = 0.1 + 0.07 * Math.sin(realT * 1.6 + i * 2.1); b.position.y = 0.55 + 0.08 * Math.sin(realT * 1.1 + i); } }   // 雋永鈎子信標:hub 輕脈動引玩家
