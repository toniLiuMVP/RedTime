...
function callArtillery() {
  ...
  showNarr("座標鎖定 · 砲彈正在路上", 2.4);
  if (!firstHeavyShown) { firstHeavyShown = true; setTimeout(() => showNarr(NARR.heavyFire, 5.5), 2600); }   // 首次:砲彈落地後浮現韌性旁白(toni 填)
...
function updateWaveHUD() { if (MODE !== "sim") { if (waveEl) waveEl.textContent = "軍營"; return; } if (waveEl) waveEl.textContent = inBreak ? (wave < 1 ? "準備" : "第 " + wave + " 波" + (isTouch ? "" : " · 清空")) : "第 " + wave + " 波"; if (scoreEl) scoreEl.textContent = score; }   // 手機:省略「· 清空」尾綴(避免窄螢幕波數列疊到雷達;清空狀態由下方提示列說明)
...
function spawnFrogmen(squads) {   // 兩棲蛙人=非戰鬥幻影(item 6 canon:他沒走的那條路):從邊緣狂奔穿過場景朝海,打不到也擋不住,不計 waveAlive
  ...
  for (let i = 0; i < total; i++) {
    ...
    const u = g.userData; u.apparition = true; u.appT = 0; u.fadeT = 0;
...
function startGaze() {
  ...
  // 融合相:光形→純黑 後,toni 親寫結局逐句浮現(居中)
  const C0 = 11200, GAP = 4200;
  ENDING_CARDS.forEach((txt, i) => {
    setTimeout(() => {
      if (MODE !== "gaze" || !lineEl) return;
      lineEl.classList.remove("show");
      setTimeout(() => { if (MODE !== "gaze" || !lineEl) return; lineEl.textContent = txt; lineEl.classList.add("cards", "show"); }, i === 0 ? 0 : 1150);
    }, C0 + i * GAP);
  });
  const endAt = C0 + ENDING_CARDS.length * GAP + 4000;   // 末句「每當我閉上眼睛」留白 4s 才收
