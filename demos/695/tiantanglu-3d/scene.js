...
(function buildPhone() {
  const px = -20, pz = 4;   // 操場西側空地(避開牛 -14,18 / 西側 barracks -30,-16)
  const root = new THREE.Group(); root.position.set(px, 0, pz); root.rotation.y = 1.0; ROOT.add(root);   // 微轉,話機正面朝操場那側
...
(function buildMotorbike() {
  ...
  const root = new THREE.Group(); root.position.set(mx, 0, mz); root.rotation.y = -0.6; ROOT.add(root);
  const part = (geo, m, x, y, z, rx, rz) => { const e = new THREE.Mesh(geo, m); e.position.set(x, y, z); if (rx) e.rotation.x = rx; if (rz) e.rotation.z = rz; e.castShadow = e.receiveShadow = true; root.add(e); return e; };
  const body = mat(0x4a2f24, { roughness: 0.6, metalness: 0.25 }), tyre = mat(0x161616, { roughness: 0.95 }), metalM = mat(0x9aa0a6, { metalness: 0.55, roughness: 0.45 }), seatM = mat(0x222229, { roughness: 0.85 }), panelM = mat(0x1a1d22, { roughness: 0.55, metalness: 0.2 }), paper = mat(0xf2ecdd, { roughness: 0.96, envMapIntensity: 0.35 });
  for (const s of [-1, 1]) { part(new THREE.CylinderGeometry(0.34, 0.34, 0.16, 16), tyre, s * 0.92, 0.34, 0, Math.PI / 2); part(new THREE.CylinderGeometry(0.12, 0.12, 0.17, 12), metalM, s * 0.92, 0.34, 0, Math.PI / 2); }   // 前後輪+輪轂
  part(new THREE.BoxGeometry(1.2, 0.22, 0.3), body, 0, 0.72, 0);                                // 車身/油箱主梁
  part(new THREE.BoxGeometry(0.62, 0.16, 0.34), seatM, -0.28, 0.86, 0);                         // 座墊
  part(new THREE.CylinderGeometry(0.05, 0.05, 0.6, 8), metalM, 0.78, 0.66, 0, Math.PI / 2.2);   // 前叉
  part(new THREE.BoxGeometry(0.36, 0.14, 0.5), panelM, 0.7, 0.92, 0);                            // 儀表台(紙條夾這裡)
  part(new THREE.CylinderGeometry(0.02, 0.02, 0.52, 6), metalM, 0.78, 1.0, 0, 0, Math.PI / 2);  // 龍頭把手橫桿
  // 儀表上夾著的紙條(微掀起的一角=有東西寫在上面)
  const note = part(new THREE.BoxGeometry(0.26, 0.012, 0.2), paper, 0.72, 1.0, 0); note.rotation.x = -0.5; note.rotation.z = 0.12;
  OBSTACLES.push([mx - 1.1, mx + 1.1, mz - 0.5, mz + 0.5]);   // 機車碰撞(item 12)
})();

/* ───────── A3 伯達樓轉角(EP13:傍晚站在伯達樓的轉角;軍營裡一道牆角立柱=夢的錯位) ───────── */
(function buildCorner() {
  const cx = -8, cz = -2;   // 操場中西側空地(避開旗桿 0,-6 / 日記 6,8)
  const root = new THREE.Group(); root.position.set(cx, 0, cz); root.rotation.y = 0.4; ROOT.add(root);
  ...
  const wallM = matT(0xb9ac8c, T.wall, 1.2, 1, { roughness: 0.93 }), capM = mat(0x8d8268, { roughness: 0.9 }), signM = mat(0x2c3a4f, { roughness: 0.8 });
  // L 形牆角:兩片牆相交成一個轉角(站在轉角=EP13 那個傍晚)
  part(new THREE.BoxGeometry(0.34, 3.0, 1.9), wallM, 0, 1.5, 0);                                 // 牆 A(沿 z)
  part(new THREE.BoxGeometry(1.9, 3.0, 0.34), wallM, 0.78, 1.5, -0.78);                          // 牆 B(沿 x,與 A 相交成 L 角)
  part(new THREE.BoxGeometry(0.5, 0.16, 0.5), capM, 0, 3.06, 0);                                 // 轉角壓頂
  part(new THREE.BoxGeometry(0.5, 0.34, 0.04), signM, 0.2, 2.2, 0.97);                           // 牆上小門牌(暗示某棟樓的轉角)
  OBSTACLES.push([cx - 0.4, cx + 1.8, cz - 1.1, cz + 0.4]);   // 牆角碰撞(item 12)
})();

/* ───────── A4 登登登的 7 秒(EP7:MSN 隱身上線等妳 7 秒後現身;辦公桌上一台舊電腦/終端機) ───────── */
(function buildTerminal() {
  const tx = 8, tz = -8;   // 操場中東側空地(避開日記 6,8 / 旗桿 0,-6)
  const root = new THREE.Group(); root.position.set(tx, 0, tz); root.rotation.y = -0.3; ROOT.add(root);
  ...
  add(new THREE.BoxGeometry(1.5, 0.07, 0.9), wood, tx, 0.82, tz);                                // 桌面(用全域 add 對齊地面)
  for (const sx of [-1, 1]) for (const sz of [-1, 1]) add(new THREE.BoxGeometry(0.08, 0.82, 0.08), wood, tx + sx * 0.66, 0.41, tz + sz * 0.36);  // 4 桌腿
  ...
  part(new THREE.BoxGeometry(0.6, 0.12, 0.28), keyM, 0.0, 0.88, 0.28, -0.05);                    // 鍵盤
...
roomBuilding(-2, 26, 0, "用餐大廳", "餐廳");      // 中山室旁;告示「餐廳」
...
(function buildWashstand() {
  const wx = -14, wz = -8;   // 操場中西側空地(避開牛 -14,18 / 轉角 -8,-2 / 電話 -20,4 / 西側 barracks -30,-16)
  const root = new THREE.Group(); root.position.set(wx, 0, wz); root.rotation.y = 0.5; ROOT.add(root);
  const part = (geo, m, x, y, z, rx) => { const e = new THREE.Mesh(geo, m); e.position.set(x, y, z); if (rx) e.rotation.x = rx; e.castShadow = e.receiveShadow = true; root.add(e); return e; };
  ...
  part(new THREE.BoxGeometry(0.9, 0.7, 0.5), postM, 0, 0.35, 0);                                    // 盥洗台座
  part(new THREE.BoxGeometry(0.96, 0.1, 0.56), porcelain, 0, 0.74, 0);                              // 台面
  part(new THREE.CylinderGeometry(0.22, 0.26, 0.16, 16), porcelain, 0, 0.82, 0.02);                 // 搪瓷臉盆
  part(new THREE.CylinderGeometry(0.04, 0.04, 0.16, 8), tapM, 0, 0.88, -0.18);                       // 水龍頭立柱
  part(new THREE.BoxGeometry(0.04, 0.04, 0.16), tapM, 0, 0.96, -0.11);                               // 水龍頭出水管
  part(new THREE.BoxGeometry(0.66, 0.86, 0.05), frameM, 0, 1.55, -0.24);                            // 鏡框
  part(new THREE.BoxGeometry(0.56, 0.76, 0.02), mirrorM, 0, 1.55, -0.215);                          // 鏡面
})();
...
const PHONE_POS = new THREE.Vector3(-20, 0, 4);      // A1 奪命連環 call 互動點(EP29 壁掛公共電話)
const MOTORBIKE_POS = new THREE.Vector3(20, 0, -4);  // A2 機車紙條互動點(EP40 妳在我機車上夾紙條)
const CORNER_POS = new THREE.Vector3(-8, 0, -2);     // A3 伯達樓轉角互動點(EP13 傍晚站在轉角)
const TERMINAL_POS = new THREE.Vector3(8, 0, -8);    // A4 登登登的 7 秒互動點(EP7 MSN 隱身上線等 7 秒)
...
function nearShrimp() { const dx = camera.position.x - SHRIMP_POS.x, dz = camera.position.z - SHRIMP_POS.z; return dx * dx + dz * dz < 6; }   // C2 餐桌那盤蝦(半徑小,要走到桌前)
...
/* ── 進入天堂路難度關卡的橫幅:清楚顯示關卡名 1 秒(不舉手夢中夢用) ── */
function showLevelBanner(text) {
  let b = document.getElementById("level-banner");
  if (!b) { b = document.createElement("div"); b.id = "level-banner"; document.body.appendChild(b); }
  b.textContent = text; b.classList.remove("out"); b.classList.add("in");
  clearTimeout(showLevelBanner._t);
  showLevelBanner._t = setTimeout(() => { b.classList.remove("in"); b.classList.add("out"); }, 1000);   // 顯示 1 秒後淡出
}
/* ── 不舉手→夢醒→掉進天堂路難度(夢中夢):難度 5 只在記憶體不持久;進場後 1 秒橫幅「天堂路難度第一關開始！」 ── */
function enterDreamLevel() {
  _preDreamDiff = settings.difficulty; settings.difficulty = 5; dreamLock = true;
  if (refreshDiffSlider) refreshDiffSlider();
  enterSim();
  setTimeout(() => showLevelBanner("天堂路難度第一關開始！"), 760);   // enterSim 的 blink 進場後浮現
}
function dreamWake() {
  ...
  if (btn) {
    btn.onclick = () => {
      ...
      setTimeout(() => {
        ...
        enterDreamLevel();   // 掉進天堂路(難度 5 只在記憶體)+ 進場橫幅「天堂路難度第一關開始！」
      }, 900);
    };
  }
}
...
function startGaze() {
  ...
  const HOLD_TOTAL = HOLD + PRE_REVEAL;
  gazeT0 = performance.now() + HOLD_TOTAL;   // 時間基準前移到「浮現起點」:HOLD+PRE 期間 t 為負,結束後 t 過 0 → 光形從 0 浮現;所有計時(心跳/reveal/字卡/收場)共用同一基準
  startHeartbeat();
  ...
  const afterHold = beginReveal;   // 留白(困難/天堂路)+ C1/C4 學長視角 pre-reveal → 她浮現(toni 親寫結局逐字不動,不插入女兒視角的敲門拍)
...
function endRun() {
  ...
  if (mendEl) mendEl.textContent = dreamLock ? "妳已經夠努力了，所以我不會對妳說加油。" : (rareDeath ? NARR.mendTwiceA : ((isRecord && deaths > 1 && isReal(NARR.mendRecord)) ? NARR.mendRecord : NARR.mend));   // 不舉手→天堂路那輪陣亡:toni 親寫釋放句(夠努力了不說加油);否則稀有死亡/結痂變奏/mend
  ...
  if (dreamLock) showNarr("妳已經夠努力了，所以我不會對妳說加油。", 5);   // 不舉手→天堂路那輪陣亡:clearly 顯示 toni 親寫的釋放句
