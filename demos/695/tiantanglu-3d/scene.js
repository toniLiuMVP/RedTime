...
addEventListener("keydown", (e) => { keys[e.code] = 1; if (e.code === "Space" || e.code === "Tab") e.preventDefault(); if (e.code in DIGIT) showWeapon(DIGIT[e.code]); else if (e.code === "KeyQ") { let _n = wi; for (let _i = 0; _i < WEAPONS.length; _i++) { _n = (_n + 1) % WEAPONS.length; if (WEAPONS[_n].owned) { showWeapon(_n); break; } } } else if (e.code === "KeyR") reload();   // Q 跳過未擁有/隱藏武器(小刀/鐵鎚),不再卡死格 beep else if (e.code === "KeyE") tryInteract(); else if (e.code === "KeyH") { beginDisarm(); } else if (e.code === "KeyB") { if (shopEl && shopEl.classList.contains("on")) closeShop(); else openShop(); } else if (e.code === "KeyG") callArtillery(); });
...
function openEmbed(ep, title) {
  ...
  reFrame.src = "../../../reader.html?embed=1#ep-" + ep;   // 同源 iframe(frame-src 'self');遊戲端音樂照常續播,讀完按返回回清單
}
...
function startGaze() {
  ...
  const beginReveal = () => {
    ...
    if (isReal(NARR.gazeEcho)) {
      ...
      setTimeout(() => { if (MODE === "gaze" && lineEl) lineEl.classList.remove("show"); }, 3900);   // 淡掉,讓「妳一直都在」主拍接上
    }
    // 光形相:她浮現,「妳一直都在」(連接句 #6)
    ...
    setTimeout(() => { if (MODE === "gaze" && lineEl) lineEl.classList.remove("show"); }, 7400);   // 提早淡掉,「十八歲」前多一段純黑留白(獨享一拍)
    // EP35 verbatim 回聲:那光的年紀,是十八歲(她站在後門的光)— 先一段純黑留白(~1.4s),再讓它獨享一拍;對齊光形微亮(見 gazeLoop lightAgeGlow)
...
function gazeLoop(ctx) {
  ...
  // 「那光的年紀,是十八歲」那拍(t∈[8.8,10.8]):光形微微一亮(三角窗,峰值約 +14%),不過曝;對齊 reveal 字卡浮現
  const lightAgeGlow = isReal(NARR.lightAge) ? 0.14 * Math.max(0, 1 - Math.abs(t - 9.8) / 1.0) : 0;
...
function updateFP(dt) {
  ...
  if (hintEl) {
    ...
    else if (MODE === "sim" && awaitDisarm && disarmT <= 0) { if (disarmReady) { hintEl.style.opacity = "1"; hintEl.classList.add("pulse"); hintEl.textContent = isTouch ? "夠了 · 輕觸「放下槍」" : "夠了 · 按 H 放下槍"; } else { hintEl.style.opacity = "0"; hintEl.classList.remove("pulse"); } }   // 放下槍=關鍵轉折,不收合
