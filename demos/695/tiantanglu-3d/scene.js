...
const beacons = [];
for (const p of [COW_POS, JACKET_POS, DIARY_POS]) { const s = new THREE.Sprite(new THREE.SpriteMaterial({ map: fireTex, color: 0xffcf90, transparent: true, opacity: 0.14, depthWrite: false, blending: THREE.AdditiveBlending, fog: false })); s.position.set(p.x, 0.55, p.z); s.scale.setScalar(1.5); ROOT.add(s); beacons.push(s); }
...
const NARR = {
  ...
  gazeEcho: "［一眼瞬間第二拍：回扣開場「每當我閉上眼睛，我就可以看見妳」，低亮度合龍。toni 親寫／拍板用不用原句］",
