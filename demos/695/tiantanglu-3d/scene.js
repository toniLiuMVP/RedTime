...
const settings = { sens: 1, vol: 0.8, chSize: 8, chGap: 4, chThick: 2, chColor: "#7dff8a", quality: 3, difficulty: 1, hicon: 0, haptic: 1 };   // 預設劇情(300HP):確保第一次玩的人撐到放下槍→一眼瞬間的情緒核心。hicon=高對比模式 / haptic=手機被擊中震動
...
function hurtPlayer(d) { if (dead || gameOver || awaitDisarm) return; if (vehicle && MODE === "sim" && !vehicle.destroyed) { vehicle.hp -= d; haptic(18); if (vehicle.hp <= 0) explodeVehicle(vehicle); else if (shake != null) shake = Math.max(shake, 0.18); return; } let _abs = 0; if (armor > 0) { _abs = Math.min(armor, d * 0.5); armor -= _abs; d -= _abs; updateArmorHUD(); } playerHP = Math.max(0, playerHP - d); if (hpEl) hpEl.textContent = Math.round(playerHP); if (dmgEl) { dmgEl.classList.toggle("armor", _abs > 0); dmgEl.style.opacity = Math.min(0.85, 0.3 + d / 35).toString(); clearTimeout(dmgEl._t); dmgEl._t = setTimeout(() => (dmgEl.style.opacity = "0"), 130); } haptic(Math.min(60, 16 + d * 1.6)); sfxHurt(); if (playerHP <= 0) endRun(); }   // 護甲吃下傷害時冷藍 flash(背心保護看得見),否則紅;手機被擊中震動(傷害越大震越久)
function endRun() {
  if (dead) return;
