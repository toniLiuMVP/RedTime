...
function updateFP(dt) {
  ...
  if (hintEl) { if (MODE === "sim" && awaitDisarm && disarmT <= 0) { hintEl.style.opacity = "1"; hintEl.textContent = isTouch ? "夠了 · 輕觸「放下槍」" : "夠了 · 按 H 放下槍"; } else if (MODE === "sim" && inBreak && !shopPause && !(shopEl && shopEl.classList.contains("on"))) { hintEl.style.opacity = "1"; hintEl.textContent = wave >= 1 ? (isTouch ? "前一波已通關 · 按「軍械」鈕購買 · 倒數後自動開打" : "前一波已通關 · 按 B 開軍備購買 · 倒數後自動開打") : (isTouch ? "準備 · 按「軍械」鈕購買裝備" : "準備 · 按 B 開軍備購買"); } else if (MODE === "hub" && isActive()) { hintEl.style.opacity = "1"; hintEl.textContent = nearVehicle() ? "按 E 上車駕駛" : nearCow() ? "按 E · 替連長顧那頭牛" : nearJacket() ? "按 E · 看那件反穿的外套" : nearDiary() ? "按 E · 翻開大兵日記" : nearRangeEntry() ? "按 E 進入「實戰模擬練習」" : (isTouch ? "搖桿走動軍營 · 按「軍械」鈕購買裝備 · 走到靶場(右前方)按 E 開始實戰" : "自由走動軍營 · 按 B 開軍械庫購買裝備 · 走到靶場(右前方)按 E 開始實戰模擬"); } else hintEl.style.opacity = "0"; }
