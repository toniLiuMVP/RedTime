...
function roomBuilding(cx, cz, ry, label) {
  ...
  // 室內昏暗暖燈(門口外亮 vs 室內暗的對比靠這盞壓不亮+暖點光)
  const il = new THREE.PointLight(0xffcf94, 2.4, 9, 2); il.position.set(0, H - 0.6, 0); g.add(il);
...
roomBuilding(16, 24, 0, "中山室");      // 門開口朝 +Z 對玩家出生點(玩家在 z=31 面 -Z 走進門)
roomBuilding(-2, 26, 0, "用餐大廳");    // 中山室旁
...
function buildOfficer(pos, torsoCol) {   // 簡易軍官立姿(primitives,同敵兵風格但靜立)
  const g = new THREE.Group(); g.position.copy(pos); g.rotation.y = 0; ROOT.add(g);   // 面 +Z 朝門口(玩家從門口 +Z 側進來對到正面)
  const skin = mat(0xc69a78, { roughness: 0.7 }), torso = mat(torsoCol, { roughness: 0.85 }), legs = mat(0x3a4a2c, { roughness: 0.9 }), boot = mat(0x1a1712, { roughness: 0.9 });
  add(new THREE.CapsuleGeometry(0.26, 0.62, 4, 8), torso, 0, 1.18, 0, g);                 // 軀幹
  add(new THREE.SphereGeometry(0.21, 14, 10), skin, 0, 1.74, 0, g);                       // 頭
  add(new THREE.CylinderGeometry(0.24, 0.26, 0.14, 12), mat(0x2c3320, { roughness: 0.8 }), 0, 1.88, 0, g);   // 軍帽
  for (const s of [-1, 1]) add(new THREE.CapsuleGeometry(0.09, 0.5, 4, 6), torso, s * 0.34, 1.16, 0.02, g);  // 雙臂(垂)
  for (const s of [-1, 1]) { add(new THREE.CapsuleGeometry(0.12, 0.56, 4, 6), legs, s * 0.14, 0.5, 0, g); add(new THREE.BoxGeometry(0.2, 0.14, 0.34), boot, s * 0.14, 0.1, 0.05, g); }   // 雙腿 + 靴
  const glow = new THREE.PointLight(0xf2f6ff, 0, 6, 2); glow.position.set(0, 1.3, 0); g.add(glow);   // 柔白輝光(intensity 由動畫脈動)
  g.userData.glow = glow;
  return g;
}
const counselorObj = buildOfficer(COUNSELOR_POS, 0x4a5238);   // 輔導長(橄欖綠)
const commanderObj = buildOfficer(COMMANDER_POS, 0x5a4632);   // 連長(深卡其)
counselorGlow = counselorObj.userData.glow; commanderGlow = commanderObj.userData.glow;

/* ───────── 1163(EP41):中山室牆上列車時刻表 / 月曆,讀出 1163 數字 → 那句話 ───────── */
(function build1163() {
  const g = new THREE.Group(); g.position.set(13.5, 0, 20.85); g.rotation.y = 0; ROOT.add(g);   // 中山室後牆內側(z≈20.5,面 +Z 朝門口,玩家進門看得到)
  const frame = mat(0x2a241c, { roughness: 0.7 }), paper = mat(0xe6decb, { roughness: 0.95, envMapIntensity: 0.3 });
  add(new THREE.BoxGeometry(1.7, 1.2, 0.08), frame, 0, 1.7, 0, g).castShadow = false;       // 時刻表外框
  add(new THREE.BoxGeometry(1.5, 1.0, 0.1), paper, 0, 1.7, 0.02, g).castShadow = false;      // 紙面(留白,讀作時刻表/月曆)
  for (let i = 0; i < 5; i++) add(new THREE.BoxGeometry(1.3, 0.04, 0.11), frame, 0, 2.05 - i * 0.16, 0.03, g).castShadow = false;   // 表格橫線
})();
...
// 難度可選性(toni #4 Q2):劇情(1)需 storyUnlocked;新手(2)破關後鎖;普通/困難/天堂路(3-5)恆可選
function diffAllowed(d) { if (d === 1) return storyUnlocked; if (d === 2) return !cleared; return d >= 3 && d <= 5; }
function clampDifficulty() { if (!diffAllowed(settings.difficulty)) { settings.difficulty = 3; saveSettings(); } }   // 目前難度不可選(破關後停在新手、或劇情未解鎖)→夾到普通(3);劇情已解鎖時本就 allowed,不會進這裡
clampDifficulty();
...
function overlayOpen() { for (const id of ["settings", "shop", "diary", "tutorial", "story", "clear", "reader-embed", "dialog"]) { const e = document.getElementById(id); if (e && e.classList.contains("on")) return true; } return false; }   // 含 dialog:輔導長/連長對白+舉手/不舉手開啟時觸控視角讓位,按鈕才吃得到 click(修敘事閘手機卡關)
...
if (isTouch) {
  ...
  addEventListener("touchmove", (e) => { if (lookId === null || overlayOpen()) return; for (const t of e.changedTouches) if (t.identifier === lookId) { const s = 0.0052 * settings.sens * touchAimMul; yaw -= (t.clientX - lpx) * s; pitch -= (t.clientY - lpy) * s; pitch = Math.max(-1.2, Math.min(1.2, pitch)); lpx = t.clientX; lpy = t.clientY; e.preventDefault(); } }, { passive: false });   // touchAimMul:開鏡且準星貼近敵人時放慢=微黏
...
bindSetting("set-vol", "vol", false, () => { if (masterGain) masterGain.gain.value = settings.vol * 0.85; });   // 與 ensureAudio 一致的 0.85 headroom(配合限制器避免削峰)
...
(function bindDifficulty() {   // toni #4:難度改「可選性」按鈕列(劇情需解鎖、新手破關後鎖、普通/困難/天堂路恆可);存設定,下次進實戰生效
  ...
  render();
  refreshDiffSlider = render;   // 解鎖劇情/破關鎖新手 當下即時重繪可選難度
})();
...
function exitVehicle() { if (!vehicle) return; const v = vehicle; camera.position.set(v.group.position.x + Math.cos(v.heading) * 3, EYE, v.group.position.z + Math.sin(v.heading) * 3); yaw = v.heading + Math.PI / 2; pitch = 0; vehicle = null; if (WEAPONS[wi]) WEAPONS[wi].group.visible = true; if (vehhudEl) vehhudEl.classList.remove("on"); }   // 下車收起載具儀表(否則 updateVehicle 不再被呼叫,#vehhud 會卡住)
...
// 雋永鈎子低調信標:三個探索點上方暖光暈輕脈動,引玩家遇見那頭牛/那件外套/那本日記(不強迫,只在 hub)
const beacons = [];
for (const p of [COW_POS, JACKET_POS, DIARY_POS, RANGE_ENTRY, COUNSELOR_POS, COMMANDER_POS]) { const s = new THREE.Sprite(new THREE.SpriteMaterial({ map: fireTex, color: 0xff3650, transparent: true, opacity: 0.42, depthWrite: false, blending: THREE.AdditiveBlending, fog: false })); s.position.set(p.x, 1.85, p.z); s.scale.set(0.55, 3.6, 1); s.raycast = () => { }; ROOT.add(s); beacons.push(s); }   // toni #2:紅線特效——可互動點(牛/外套/日記/靶場/輔導長/連長)立一道脈動紅光柱;raycast no-op(不是射擊目標,且 Sprite.raycast 需 raycaster.camera→會 crash)
...
const NARR = {
  ...
  gaze: "妳一直都在。",
  // 敲不開的門 scaffold(Claude 連接句,非小說正文):玩家伸手想敲門卻不能干預
  knockScaffold: "（這扇門，敲不開。）",
  // EP41 verbatim:把「知道」放下,選擇相信(從被動「不必再打」翻成主動「我願意相信」)
  believe: "所以我把「知道」放下。我決定，跟把拔做同一件事。我也相信。",
  // EP28 verbatim:命運要人先伸手,紅線才找得到回來的路
  reach1: "命運不是命定，它要人先伸手。",
  reach2: "是我們先伸手，是「選擇」讓紅線成形，也因此，那縷紅線才找得到回來的路。",
...
let metCounselor = false, raisedHand = false, counselorSeen1163 = false;
...
/* ── 1163(EP41):先只顯示數字 1163(玩家也讀不懂),幾秒後淡入那句話 ── */
function reveal1163() {
  const el = document.getElementById("eleven63"); if (!el) return;
  counselorSeen1163 = true;
  const num = document.getElementById("eleven63-num"), line = document.getElementById("eleven63-line");
  el.classList.add("on"); if (num) { num.textContent = "1163"; num.style.opacity = "1"; } if (line) { line.style.opacity = "0"; line.textContent = "她沒有辦法把那七天，打成一句話。她只能告訴把拔：我正在去找你的路上。"; }   // EP41 verbatim 片段(她沒有辦法把那七天，打成一句話 / 我正在去找你的路上)+ 連接句
  clearTimeout(reveal1163._t1); clearTimeout(reveal1163._t2);
  reveal1163._t1 = setTimeout(() => { if (line) line.style.opacity = "1"; }, 2600);            // 幾秒後淡入那句話
  reveal1163._t2 = setTimeout(() => { el.classList.remove("on"); if (num) num.textContent = ""; }, 11000);   // 整段約 11s 後淡出
}
/* ── 輔導長:玩家內心話 → 輔導長回話(笑而不答)→ 牆上時刻表 1163 揭示 ── */
function talkCounselor() {
  ...
  setTimeout(reveal1163, 900);   // 對白後牆上時刻表浮現 1163
}
...
function talkCommander() {
  ...
  showDialog([
    ...
    { label: "舉手", onClick: () => { raisedHand = true; showDialog([{ who: "", text: "伯達樓那個轉角，那天我哭紅了雙眼。我就站在那邊，好久好久。" }], null); } },
...
function closeEmbed() { if (reEl) reEl.classList.remove("on"); if (reFrame) reFrame.src = "about:blank"; }   // 卸載 iframe 釋放資源
...
function updateMusicToggleLabels() {   // 結局選單/閱讀的音樂鈕標籤隨 gazeMusic 狀態
  ...
  const rm = document.getElementById("re-music"); if (rm) rm.textContent = on ? "🔊" : "🔇";
}
...
const _reMusicBtn = document.getElementById("re-music"); if (_reMusicBtn) _reMusicBtn.addEventListener("click", toggleGazeMusic);
...
/* ── 不舉手 → 夢醒序幕:全白→「原來是夢......」→「起床」鈕→直接掉進天堂路(難度5)且鎖死難度(夢中夢) ── */
function dreamWake() {
  const ov = document.getElementById("dreamwake"); if (!ov) { _preDreamDiff = settings.difficulty; settings.difficulty = 5; dreamLock = true; enterSim(); return; }   // 浮層缺失也別卡住:直接進(難度只在記憶體,不持久)
  ...
  if (btn) {
    btn.onclick = () => {
      ...
      setTimeout(() => {
        ...
        _preDreamDiff = settings.difficulty; settings.difficulty = 5; dreamLock = true;   // 掉進天堂路 + 鎖死難度(只在記憶體,不 saveSettings→不寫進 localStorage,夢醒後還原)
        if (refreshDiffSlider) refreshDiffSlider();
        enterSim();
      }, 900);
    };
...
let awaitDisarm = false, disarmT = 0, survivedAt = -99;   // disarmT>0:放下槍動作進行中;survivedAt:撐過那刻時戳(讓「撐過了」旁白先獨處,放下槍提示稍後才浮現)
...
const ENDING_CARDS = [
  { txt: "不論三年、還是五年，\n在跟你不期而遇的那天。", hold: 4200 },
  { txt: "「學妹！」\n「學長！」", hold: 3000 },
  { txt: "雖然後來事實的真相是，\n不是三年，也不是五年，\n是十七年後。", hold: 5400 },
  { txt: "「等待的時候，也算在約會的時間裡。」", hold: 4200 },
  { txt: "妳後來問我：「你是不是在賭？」\n我說：「我只是相信『相信的力量』。」", hold: 5200 },
  { txt: "「每當我閉上眼睛，我就可以看見妳。」", hold: 6000 },
];
...
function startHeartbeat() {   // 動態心跳:reveal 慢而沉,連接句那拍收緊(被釘住),非節拍器
  ...
  const beat = () => {
    ...
    if (t < 2.6) { iv = 1320; v0 = 0.34; } else if (t > 5.6 && t < 8.6) { iv = 880; v0 = 0.4; }   // 光形 reveal 慢沉 / 連接句浮現收緊
    try { tone(58, 42, 0.16, v0, "sine"); setTimeout(() => { try { tone(50, 36, 0.13, v0 * 0.68, "sine"); } catch (e) { } }, 230); } catch (e) { }
...
function startGaze() {
  ...
  // 敲不開的門(EP41 升維相信):reveal 前先在純黑裡放一拍 — 看得見卻按不下去的「敲門」鈕(伸手卻不能干預)→ 把「知道」放下選擇相信 → 先伸手紅線才找得到回來的路
  const doorOn = isReal(NARR.believe) && isReal(NARR.reach1) && isReal(NARR.reach2);
  const DOOR = doorOn ? 16400 : 0;   // 門拍總長(scaffold + 相信 + 先伸手兩句,各自留白讀完);未填則跳過
  gazeT0 = performance.now() + HOLD + DOOR;   // 時間基準前移到「浮現起點」:HOLD+DOOR 期間 t 為負,結束後 t 過 0 → 光形從 0 浮現;所有計時(心跳/reveal/字卡/收場)共用同一基準
  startHeartbeat();
  requestAnimationFrame(() => ov.classList.add("closing"));        // 閉眼(純黑留白開始)
  const knockEl = document.getElementById("gaze-knock");
  const doorBeat = (onDone) => {   // 在純黑裡:敲門鈕浮現(按不下去)+ scaffold,再淡到 EP41 相信、EP28 先伸手;不啟動 gazeLoop,畫面維持純黑
    if (!doorOn || !lineEl) { onDone(); return; }
    const set = (txt, ms) => setTimeout(() => { if (MODE !== "gaze") return; lineEl.classList.remove("show"); setTimeout(() => { if (MODE !== "gaze") return; lineEl.textContent = txt; lineEl.classList.remove("echo", "cards"); lineEl.classList.add("show"); }, 700); }, ms);
    if (knockEl) { knockEl.classList.add("on"); requestAnimationFrame(() => { if (MODE === "gaze") knockEl.classList.add("in"); }); }   // 敲門鈕淡入(disabled+pointer-events:none,按不下去)
    if (lineEl) { lineEl.textContent = NARR.knockScaffold; lineEl.classList.remove("echo", "cards"); requestAnimationFrame(() => { if (MODE === "gaze") lineEl.classList.add("show"); }); }   // （這扇門，敲不開。）
    set(NARR.believe, 4600);   // EP41:把「知道」放下,選擇相信
    set(NARR.reach1, 9000);    // EP28:命運要人先伸手
    set(NARR.reach2, 12200);   // EP28:先伸手紅線才找得到回來的路
    setTimeout(() => { if (knockEl) knockEl.classList.remove("on", "in"); if (lineEl) { lineEl.classList.remove("show"); lineEl.textContent = NARR.gaze; } onDone(); }, DOOR);   // 門拍收:撤敲門鈕、line 復位成「妳一直都在」,接 reveal
  };
  const beginReveal = () => {
    if (MODE !== "gaze") return;
    setTimeout(() => ov.classList.add("open"), 2600);              // 睜眼(留電影黑邊):延到她開始浮現那刻,letterbox 對齊 reveal,別太早洩掉閉眼張力
    // 光形相:她浮現,「妳一直都在」(連接句 #6)
    setTimeout(() => { if (MODE === "gaze" && lineEl) { lineEl.textContent = NARR.gaze; lineEl.classList.remove("echo", "cards"); lineEl.classList.add("show"); } }, 4000);
    setTimeout(() => { if (MODE === "gaze" && lineEl) lineEl.classList.remove("show"); }, 8000);   // 淡掉,讓 EP35 那句接上
    // EP35 verbatim 回聲:那光的年紀,是十八歲(她站在後門的光)— 對齊背光相,reveal 那拍的回響
    setTimeout(() => { if (MODE === "gaze" && lineEl && isReal(NARR.lightAge)) { lineEl.textContent = NARR.lightAge; lineEl.classList.remove("echo", "cards"); lineEl.classList.add("show"); } }, 8800);
    setTimeout(() => { if (MODE === "gaze" && lineEl) lineEl.classList.remove("show"); }, 10800);   // 淡掉,讓光形隨之淡入純黑
    setTimeout(() => stopHeartbeat(), 11000);   // 光形相結束→心跳收,文字相只留一眼瞬間樂
    // 融合相:光形→純黑 後,toni 親寫結局逐句浮現(居中);重句多停輕句快走(呼吸);末句套 echo 低亮度回返=首尾合龍(回扣 cold-open 同句)
    let ct = 11200; const lastI = ENDING_CARDS.length - 1;
    ENDING_CARDS.forEach((card, i) => {
      setTimeout(() => {
        if (MODE !== "gaze" || !lineEl) return;
        lineEl.classList.remove("show");
        setTimeout(() => { if (MODE !== "gaze" || !lineEl) return; lineEl.textContent = card.txt; lineEl.classList.remove("echo"); lineEl.classList.add("cards", "show"); if (i === lastI) lineEl.classList.add("echo"); }, i === 0 ? 0 : 1150);
      }, ct);
      ct += card.hold;
    });
    setTimeout(() => showCloseEyes(), ct + 5000);   // 末句後 5s 浮現「閉上眼睛」按鈕(音樂不停),收束權交給玩家點擊
    gazeLoop(ctx);   // gazeT0 已在開頭設好(= 浮現起點),beginReveal 此刻起跑 t≈0
  };
  const afterHold = () => doorBeat(beginReveal);   // 留白(困難/天堂路)→ 敲不開的門拍 → 她浮現
  if (HOLD > 0) setTimeout(afterHold, HOLD); else afterHold();
}
function gazeLoop(ctx) {
  ...
  for (const p of gazeFigure) {
    ...
    const a = reveal * fade * breathe * (0.5 + 0.5 * Math.sin(t * 1.1 + p.ph)) * (1 + clarity * 0.6), rr = p.r * push * (1 + 0.2 * Math.sin(t * 2 + p.ph));
...
function onCloseEyes() {   // 點「閉上眼睛」:畫面淡出全暗(音樂不停)→ 5s →彩虹光→結局選單
  ...
  if (cv) { cv.style.transition = "opacity 2s"; cv.style.opacity = "0"; }
  if (lineEl) { lineEl.style.transition = "opacity 1.5s"; lineEl.style.opacity = "0"; }
  setTimeout(() => {   // 全暗後再 5s(toni 規格)→ 一陣彩虹光,音樂續播
    if (MODE !== "gaze") { _gazeClosing = false; return; }   // 異常離開 gaze:解鎖,下次進結局按鈕仍可用
    const rb = document.getElementById("rainbow");
    if (rb) { rb.classList.remove("burst"); void rb.offsetWidth; rb.classList.add("burst"); }
    setTimeout(() => { if (MODE === "gaze") finishToClearMenu(); }, 1200);   // 彩虹光升起時接到結局選單(光還燒著);加 MODE 守衛防重入
  }, 7000);   // 2s 淡黑 + 5s 留白
}
function finishToClearMenu() {   // 收束到結局選單:重置世界回 hub + 開破關紀念;gazeMusic(一眼瞬間)持續不停(由結局選單/閱讀的音樂鈕或「再玩/回全集」才停)
  ...
  const knockEl = document.getElementById("gaze-knock"); if (knockEl) knockEl.classList.remove("on", "in");   // 防殘留:重玩時敲門鈕復位
  if (rb) rb.classList.remove("burst");
...
function endRun() {
  ...
  const rareDeath = !rareDeathDone && ceremony && deaths >= 3 && Math.random() < 0.34 && isReal(NARR.mendTwiceA) && isReal(NARR.mendTwiceB);
  if (rareDeath) rareDeathDone = true;
  if (mendEl) mendEl.textContent = rareDeath ? NARR.mendTwiceA : ((isRecord && deaths > 1 && isReal(NARR.mendRecord)) ? NARR.mendRecord : NARR.mend);   // 稀有死亡:死亡字卡顯示阿姨那句氣話;否則 W6 結痂變奏/退回 mend
  ...
  _reviveShowAunt = ceremony && !rareDeath;   // 重生時放 EP21「阿姨每次都會再回來一次」(把廉價重來翻成:她一次次回來,所以你再站起來);稀有死亡那次已有自己的旁白,不疊
...
const COLLIDERS = [
  ...
  ...ROOM_COLLIDERS,           // 中山室/用餐大廳 牆段(留門洞,可走進去)
];
...
function updateFP(dt) {
  ...
  const disarmReady = MODE === "sim" && awaitDisarm && disarmT <= 0 && (realT - survivedAt > 2.2);   // 撐過那刻先讓「撐過了」+ survived 旁白獨處 ~2.2s,放下槍提示才浮現(不稀釋轉折)
  ...
  if (hintEl) {
    ...
    const _nearI = (MODE === "hub" && isActive()) ? (nearVehicle() ? "上車駕駛" : nearCounselor() ? "和輔導長談談" : nearCommander() ? "和連長談談" : nearCow() ? "替連長顧那頭牛" : nearJacket() ? "看那件反穿的外套" : nearDiary() ? "翻開大兵日記 · 看戰前的故事" : nearRangeEntry() ? "進入「實戰模擬練習」" : null) : null;
    if (_tbAct) _tbAct.classList.toggle("glow", !!_nearI);   // toni #2:靠近可互動點→「使用」鈕發紅光提示
    const _hintsOn = document.body.classList.contains("hints-on");   // 提示預設收合(toni #2);靠近互動點/放下槍 例外一定顯示
    if (MODE === "sim" && awaitDisarm && disarmT <= 0) { if (disarmReady) { hintEl.style.opacity = "1"; hintEl.classList.add("pulse"); hintEl.textContent = isTouch ? "夠了 · 輕觸「放下槍」" : "夠了 · 按 H 放下槍"; } else { hintEl.style.opacity = "0"; hintEl.classList.remove("pulse"); } }   // 放下槍=關鍵轉折,不收合
  ...
  for (let i = 0; i < beacons.length; i++) { const b = beacons[i]; b.visible = MODE === "hub"; if (b.visible) { b.material.opacity = 0.32 + 0.2 * Math.sin(realT * 1.8 + i * 2.1); b.position.y = 1.85 + 0.12 * Math.sin(realT * 1.1 + i); } }   // toni #2:紅光柱在 hub 脈動引玩家走向可互動點
  ...
  if (disarmT > 0) { disarmT -= dt; if (disarmT < 0) disarmT = 0; const e = 1 - disarmT / 0.85, s = e * e; disDipY = -s * 0.75; disRotX = s * 1.15; pitch += (-0.34 - pitch) * Math.min(1, dt * 3.5); if (disarmT === 0) startGaze(); }   // 放下槍:槍垂下+槍口下傾(ease-in)+ 鏡頭順勢低頭看放下的手(韓劇運鏡),到底進一眼瞬間
