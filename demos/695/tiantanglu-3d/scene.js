...
function hitEnemy(g, dmg, head, point) {
  ...
  dmg = Math.round(dmg);
  bloodAt(g.position.clone().setY(head ? 1.66 : 1.2)); u.hp -= dmg; u.flinch = 0.16; u.coverT = 1.4;
  // 武器去爽(W5-A):無傷害數字 / 無連殺 / 無分數,殺只是消耗不是獎勵(夢裡改不了的徒勞,非 power fantasy)
  if (u.hp <= 0) {
    u.dead = true; u.deadT = 0; kills++;   // kills 仍內部計數供波數結算 / 死亡字幕
...
function updateEnemies(dt) {
  ...
  for (let i = enemies.length - 1; i >= 0; i--) {
    ...
    if (u.dead) { u.deadT += dt; g.rotation.x = Math.max(-1.55, g.rotation.x - dt * 4.5); if (u.deadT > 5) { scene.remove(g); enemies.splice(i, 1); } continue; }
...
function updateFP(dt) {
  ...
  // 自動射擊(步槍/機槍按住)
  if (!dead && mouseDown && w.type === "auto" && document.pointerLockElement === canvas && realT - lastShot >= w.rate) { lastShot = realT; fire(); }
  // 槍口焰
