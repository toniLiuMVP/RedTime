...
function dialogueDefaultRect() {
  ...
  const y = isMobile()
    ...
    : Math.max(pad, Math.round(vh * 0.18));
...
function ensureTranscriptControls(panel) {
  ...
  const allowMaximize = panel.id === "transcript";
  if (allowMaximize && !panel.maxButton) {
...
function registerPanel(id, config) {
  ...
  if (panel.resizable) {
    panel.handle?.addEventListener("dblclick", (event) => {
      event.preventDefault();
      toggleTranscriptMaximize(panel.id);
    });
  }
...
export function initPanelSystem() {
  ...
  if (!initialized) {
    ...
    registerPanel("dialogue", {
      element: document.querySelector(".dialogue-card"),
      handle: document.getElementById("dialogue-title"),
      resizable: true,
      minW: 320,
      minH: 220,
      getDefaultRect: dialogueDefaultRect,
    });
