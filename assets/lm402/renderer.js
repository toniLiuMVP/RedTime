...
function createPerson(spec) {
  ...
  if (spec.female) {
    // Long jeans covering the full leg — high-waisted denim style
    const jeansMat = new THREE.MeshPhysicalMaterial({
      color: "#1e3a56", roughness: 0.46, metalness: 0.04,
      clearcoat: 0.14, clearcoatRoughness: 0.3
    });
    const jeansL = new THREE.Mesh(new THREE.BoxGeometry(0.14, 0.66, 0.19), jeansMat);
    jeansL.position.set(-0.085, 0.36, 0.008);
    const jeansR = jeansL.clone();
    jeansR.position.x = 0.085;
    // Waistband
    const waistband = new THREE.Mesh(new THREE.BoxGeometry(0.35, 0.05, 0.27), jeansMat);
    waistband.position.set(0, 0.80, 0.01);
    // Cuffs at the ankle
    const cuff = new THREE.Mesh(new THREE.BoxGeometry(0.15, 0.03, 0.20), jeansMat);
    cuff.position.set(-0.085, 0.14, 0.008);
    const cuffR = cuff.clone();
    cuffR.position.x = 0.085;
    group.add(jeansL, jeansR, waistband, cuff, cuffR);
  }
...
export function createLm402Scene(canvas) {
  ...
  // Junior height: only slightly shorter than senior (scale 1.06 vs 1.08)
