...
const RUNTIME_VERSION = 'runtime-v53-20260528';  // bumped: tech-demo realism push v4 — SSAO post-process (8 stratified hemisphere samples per pixel against existing rtMain depth texture, rotated by per-pixel hash to break banding, separable Gaussian denoise pass, multiply into final composite at strength 0.48); procedural Sobel-derived normal map for skin (matching pore canvas, 1.4 strength, applied at normalScale 0.22 — subtle dermal hint not pebble); head subdivision bumped to IcosahedronGeometry detail 5 (~2560 tri/head, 4x density vs v3, eliminates close-up facets); window emissive boosted (toneMapped:false white-hot core + additive halo plate behind for atmospheric scattering); Verlet hair physics on junior long strands + side strands (spring stiffness 44-68 / damping 0.93-0.97 / prev-position based velocity, head rotation Y velocity feeds X drag, breath wobble + ambient noise + clamped displacement, rotation responds to lateral whip); fixed SSAO tSSAO uniform fallback to rtMain texture when off
...
const RUNTIME_PRECACHE_URLS = [
  ...
  /* AI 3D 競賽選手 1 號 — LM402 一眼瞬間 narrative cinema */
