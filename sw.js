...
const RUNTIME_VERSION = 'runtime-v51-20260527';  // bumped: tech-demo realism push v3 — PMREM IBL with procedural sky envMap (warm-window-light gradient + cold corridor tint convolved into roughness-prefiltered cubemap), MeshPhysicalMaterial reflections now bounce real environment light; anisotropic hair shader via onBeforeCompile (view-space tangent varying from cylinder local Y, Gaussian band perpendicular to strand = Marschner / Kajiya-Kay surrogate, double-highlight for layered shine); iris ShaderMaterial with view-dependent UV parallax (fake refraction through cornea bulge) + limbal ring darkening + specular rim; eye blink animation (per-character phase, 110ms double-peak, upper+lower lid scale+pos) + saccade (1.8s period, hash-based random XY iris/pupil jitter); depth-aware DOF post-process (DepthTexture on rtMain, separable Gaussian blur to rtDofA, final composite mixes sharp vs blur by linear depth distance from focus); dynamic focus depth tracking cam.lookAt target; sheen cloth materials; cleaned up close-up cam lock
