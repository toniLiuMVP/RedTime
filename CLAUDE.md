...
| 全域變數 | 用途 |
|---|---|
| `window.__POSTFX__.tuning` | 後製管線（Bloom / DOF / CA / Grain / Color Grade / Lens Dirt / Rain / God Rays / Lens Flare） |
| `window.__JUNIOR_RIG__.state` | 表情 + 互動（blink/mouthOpen/smile/browRaise/lookDir + autoEyeTrack/Saccade/Micro/HeadWobble/GazeAI + blush/sweat） |
| `window.__CLOTH_RIG__.tuning` | 布料動態（spring/damping/wind） |
| `window.__CONSC_LIGHTS__` | **雙時空 only**：意識菜市場光柱（setIntensity / inspectVoices） |
| `window.__LM402_YEAR__` | 預留變數（目前固定 "2005"，未來支援動態） |
