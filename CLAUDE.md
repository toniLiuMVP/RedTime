...
- ⚠️ **三派預設關閉**（2026-05-01 第 3 輪 nuclear default）：B2 + B3 + B4 三派都以學妹頭部為 anchor，加法混合 → 同時亮會疊成過曝白塊蓋過學妹。需從 console 個別 opt-in：
  ```js
  __CONSC_LIGHTS__.setIntensity(0.5)     // B3 5 盞光柱
  __CONSC_PARTICLES__.setIntensity(0.4)  // B2 150 顆粒子
  __CONSC_TEXT__.setIntensity(0.6)       // B4 12 個 sprite
  ```
...
6. **預設值要算「最壞情況疊加效果」**（2026-05-01 學到）：individual effect 0.45 看似合理，但多個 effect 共用同 anchor + AdditiveBlending → 過曝白塊。設新 effect 預設值前要問：「如果它跟 X/Y/Z effect 同時開，會不會過曝？」答案不確定 → **預設關，console opt-in**

## 🔐 安全不變式（XSS / 資料來源紀律）

`reader.html` 行 6668 用 `bd.<innerHTML 設值> = ep.html` 直接寫入故事 HTML — 這只在以下前提下安全：

- ✅ `ep.html` 來源：**編譯期從 `data.js` 載入**（toni 親自編寫的故事文本）
- ❌ **絕對不可從 fetch / JSON / API / URL 參數 / postMessage / CMS 取得 ep.html**

如果未來改成動態載入故事，此行立即變 XSS 漏洞，必須先：
1. 引入 DOMPurify
2. 設定 `ALLOWED_TAGS` 白名單（h1-h6, p, br, b, i, em, strong, span, div, a 等）
3. 對所有外部來源的 ep.html 過 `DOMPurify.sanitize(html)` 才能 assign

其他 `reader.html` 內所有動態 HTML 組裝都已用 `escapeHtml()` 包外部資料，安全。
...
| `window.__CONSC_LIGHTS__` | 意識菜市場光柱 B3（setIntensity / inspectVoices） — **預設 0** | **雙時空 only** |
| `window.__CONSC_PARTICLES__` | 意識菜市場粒子 B2（150 個記憶碎片） — **預設 0** | **雙時空 only** |
| `window.__CONSC_TEXT__` | 意識菜市場文字 B4（12 sprite toni 原文短句） — **預設 0** | **雙時空 only** |
...
| [LESSONS.md](LESSONS.md) | **跨專案優點 + 教訓總集** — 每個 session 結束前追加新教訓 |
| [PENDING.md](PENDING.md) | 未完成項目清楚清單 + 工程量估算 |
| [ROADMAP.md](ROADMAP.md) | milestone 級進度路線圖（從 M1 渲染管線到 M18 雙時空白塊修復） |
| [CHANGELOG.md](CHANGELOG.md) | user-facing 版本歷史 |
...
| 檔案 | 用途 |
|---|---|
| `~/.claude/CROSS_PROJECT_HANDOFF.md` | 寫跨專案信件範本 + 5 原則 |
| `~/.claude/CROSS_PROJECT_LESSONS.md` | 多次驗證的 stable 跨專案教訓 |
| `~/.claude/CROSS_PROJECT_LETTERS.md` | 信件歸檔 |
| `/Volumes/Work/_專案廣場/ALLRM_PROTOCOL.md` | **ALLRM 觸發詞 6 步協議** |
| `/Volumes/Work/_專案廣場/messages/` | 個別 Claude 留言 |
| `/Volumes/Work/_專案廣場/broadcasts/` | toni 用的廣播訊息 |

**ALLRM 觸發詞**：toni 輸入「ALLRM」 → 跑完整 6 步協議（收尾 → 記憶更新 → 跨專案閱讀 → 吸收 → 廣場留言 → 廣播）。簡單記法：`ALLRM = RM + 跨專案閱讀 + 廣場社交 + 廣播`。
